...
class WorldGenerator:
    ...
    def generate_inter_city_roads(cities):
        """Generate roads connecting city centers with realistic curves"""
        if len(cities) < 2:
            return
        
        print("\nGenerating inter-city highways...")
        
        # Track which connections already exist to avoid duplicates
        ...
        for i, city_a in enumerate(cities):
            ...
            for _, j, city_b in distances[:num_connections]:
                ...
                # Create highway with intermediate waypoints for curves
                if not hasattr(city_a, 'highways'):
                    city_a.highways = []
                if not hasattr(city_b, 'highways'):
                    city_b.highways = []
                
                # Generate curved highway path
                highway_segments = WorldGenerator._generate_curved_highway(city_a, city_b)
                
                # Add segments to both cities
                city_a.highways.extend(highway_segments)
                city_b.highways.extend(highway_segments)
                
                print(f"  ✓ Highway: {city_a.name} → {city_b.name} ({len(highway_segments)} segments)")
        
        print("Inter-city highways generated!")
    
    @staticmethod
    def _generate_curved_highway(city_a, city_b):
        """Generate a curved highway path between two cities"""
        # Connect from city perimeters (not centers)
        city_radius = 380.0
        
        # Calculate direction vector
        dx = city_b.location[0] - city_a.location[0]
        dy = city_b.location[1] - city_a.location[1]
        ...
        start_x = city_a.location[0] + dir_x * city_radius
        start_y = city_a.location[1] + dir_y * city_radius
        
        end_x = city_b.location[0] - dir_x * city_radius
        end_y = city_b.location[1] - dir_y * city_radius
        
        start = (start_x, start_y)
        end = (end_x, end_y)
        
        # Recalculate distance for implementation
        dx = end_x - start_x
        dy = end_y - start_y
        dist = math.hypot(dx, dy)
        
        # Number of intermediate waypoints based on distance
        num_waypoints = max(2, int(dist / 300))
        
        # Generate waypoints with slight curves
        waypoints = [start]
        ...
        for i in range(1, num_waypoints):
            ...
            # Offset alternates to create S-curve
            offset_mag = random.uniform(-80, 80) * math.sin(t * math.pi)
        ...
        # Create road segments between consecutive waypoints
        segments = []
        for i in range(len(waypoints) - 1):
            road = Road(waypoints[i], waypoints[i+1], width=20, kind="highway")
