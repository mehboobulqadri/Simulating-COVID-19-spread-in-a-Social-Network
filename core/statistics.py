...
class StatisticsManager:
    ...
    def update(self, cities, current_time, engine=None):
        ...
        if engine:
            # Use NumPy engine for fast counting
            # engine.state is an array of int values corresponding to State enum
            unique, ucounts = np.unique(engine.state, return_counts=True)
            count_dict = dict(zip(unique, ucounts))
